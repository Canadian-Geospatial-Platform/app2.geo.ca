...
const GeoSearch = (
    ...
    const ksToggle = (kso: boolean) => {
        ...
        const btn = cgpv.api
            .map("mapWM")
            .appBarButtons.getAppBarButtonPanelById("searchBtn");
        const panel = btn?.panel;
        if (panel) {
            panel.width = kso ? 1200 : 500;
            panel.changeContent(<SearchPanel ksonly={kso} />);
        }
    };
                ...
                >
                    ...
                    >
                        <FormControlLabel
                            value="start"
                            control={
                                <Switch color="primary" onChange={() => ksToggle(!ksOnly)} />
                            }
                            label={t("appbar.keywordonly")}
