...
import { analyticGet, analyticPost } from "../../common/analytic";
...
const MetaDataPage = (props) => {
    const [metadataState] = useState(
        JSON.parse(localStorage.getItem("metadataState"))
    );

    const queryParams = getQueryParams(location.search);
    const stateKO =
        location.state && location.state.stateKO
            ? location.state.stateKO
            : metadataState && metadataState.stateKO
                ? metadataState.stateKO
                : false;
    const stateKeyword =
        location.state && location.state.stateKeyword
            ? location.state.stateKeyword
            : metadataState && metadataState.stateKeyword
                ? metadataState.stateKeyword
                : "";
    const statePn =
        location.state && location.state.statePn
            ? location.state.statePn
            : metadataState && metadataState.statePn
                ? metadataState.statePn
                : 1;
    const stateBounds =
        location.state && location.state.stateBounds
            ? location.state.stateBounds
            : metadataState && metadataState.stateBounds
                ? metadataState.stateBounds
                : undefined;
    const backArr =
        location.state && Array.isArray(location.state.backId)
            ? location.state.backId
            : [];
    ...
    const rid =
        queryParams && queryParams.id
            ? queryParams.id.trim()
            : metadataState && metadataState.id
                ? metadataState.id
                : "";
    ...
    const handleSearch = (id) => {
        ...
        axios
            .get(
                `${EnvGlobals.APP_API_DOMAIN_URL}${EnvGlobals.APP_API_ENDPOINTS.METADATA}`,
                { params: searchParams }
            )
            ...
            .then((data) => {
                ...
                axios
                    .get(
                        `${EnvGlobals.APP_API_DOMAIN_URL}${EnvGlobals.APP_API_ENDPOINTS.COLLECTIONS}`,
                        { params: { id } }
                    )
    ...
    const showMapping = () => {
        const cmapping =
            loadState() !== undefined ? loadState().mappingReducer.mapping : [];
    ...
    const viewOnMap = (event, resultid) => {
        viewParams.event = "map";
        //analyticPost(viewParams);
        event.preventDefault();
        window.location.href = `/map?rvKey=${resultid}`;
    };
    ...
    const getInMapping = () => {
        const cinMapping =
            loadState() !== undefined && rid !== ""
                ? loadState().mappingReducer.mapping.findIndex((m) => m.id === rid) > -1
                : false;
        const ammButton = document.getElementById("addMyMap");
        if (ammButton) {
            ammButton.className = cinMapping
                ? "btn btn-search btn-added"
                : "btn btn-search";
            ammButton.innerText = cinMapping
                ? t("page.addedtomymap")
                : t("page.addtomymap");
        }
    };
    ...
    const textParagraphOutput = (paraText) => {
        ...
        return paraArray.map((pt, i) =>
            pt.trim().length > 0 ? (
                <p
                    key={`pt${i}`}
                    dangerouslySetInnerHTML={{ __html: convertDesc(pt) }}
                />
            ) : (
                <br key={`pt${i}`} />
            )
    ...
    useEffect(() => {
        if (metaresult !== null && rid !== "" && !metadataState) {
            window.location.href = `/result/${language}/${encodeURI(
                metaresult.mappingtitle[language]
                    .trim()
                    .toLowerCase()
                    .replaceAll(" ", "-")
            )}?id=${encodeURI(rid.trim())}&lang=${language}`;
        }
    }, [metaresult, language, rid]);
    ...
    return (
        ...
        >
            {loading ? (
                ...
                <p className="d-flex justify-content-center">{t("page.noresult")}</p>
            ) : (
                [metaresult].map((result, rmIndex) => {
                    ...
                    const graphicOverview =
                        formattedgraphicOverview !== null && formattedgraphicOverview !== ""
                            ? formattedgraphicOverview
                            : { overviewFileName: "" };
                    const imageFile = graphicOverview
                        .map((o) => o.overviewFileName)
                        .join(",");
                    const options = formattedOption
                        ...
                        .map((option) => {
                            ...
                            return {
                                ...
                                type: desc[0],
                                format: desc[1],
                                language: t(`page.${desc[2].toLowerCase().replace(",", "")}`),
                                url: option.url,
                            };
                        });
                    ...
                    const mapButtonClass = activeMap
                        ? "btn btn-search"
                    ...
                    const spatialRepresentation =
                        result.spatialRepresentation.split(";")[langInd];
                    const dist = Math.max(
                    ...
                    const showDisclaimer =
                        Array.isArray(useL) &&
                        licenceOrgs[language].findIndex(
                            (p) => p.toLowerCase() === useL[2].trim().toLowerCase()
                        ) > -1;
                    ...
                    return (
                        <div
                            key={`rm-${rmIndex}`}
                            className="container-search-result container-search-result-two-col"
                        >
                            <Helmet>
                                <title>{result.title} - GEO.CA Viewer</title>
                                <link
                                    rel="alternate"
                                    hrefLang={`${language}-ca`}
                                    href={encodeURI(
                                        `https://app.geo.ca/result/${language}/${encodeURI(
                                            result.title.trim().toLowerCase().replaceAll(" ", "-")
                                        )}/`
                                    )}
                                />
                                ...
                                <meta
                                    ...
                                    content={`${window.location.origin}/assets/img/GeoDotCaBanner.jpg`}
                                />
                            ...
                            <div className="row no-gutters">
                                ...
                                <main className="col col-12 col-xl-8 main">
                                    ...
                                    {/* About */}
                                    <section
                                        id="search-result-about"
                                        className="sec-search-result sec-search-result-about"
                                    >
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <p>
                                                                {t("page.dcp2.text1")}
                                                                <a
                                                                    className="table-cell-text-link"
                                                                    href={t("page.dcp2.link1.url")}
                                                                    target="_blank"
                                                                >
                                                                    {t("page.dcp2.link1.text")}
                                                                </a>
                                                                {t("page.dcp2.text2")}
                                                                <a
                                                                    className="table-cell-text-link"
                                                                    href={t("page.dcp2.link2.url")}
                                                                    target="_blank"
                                                                >
                                                                    {t("page.dcp2.link2.text")}
                                                                </a>
                                                                {t("page.dcp2.text3")}
                                                            </p>
                                                        </td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <p>
                                                                ...
                                                                <a
                                                                    className="table-cell-text-link"
                                                                    href={t("page.dwh2.link1.url")}
                                                                    target="_blank"
                                                                >
                                                                    {t("page.dwh2.link1.text")}
                                                                </a>
                                                                {t("page.dwh2.text2")}
                                                                <a
                                                                    className="table-cell-text-link"
                                                                    href={t("page.dwh2.link2.url")}
                                                                    target="_blank"
                                                                >
                                                                    {t("page.dwh2.link2.text")}
                                                                </a>
                                                                {t("page.dwh2.text3")}
                                                            </p>
                                                        </td>
                                    ...
                                    {result.related.length > 0 && (
                                        <section
                                            id="search-result-related-products"
                                            className="sec-search-result sec-search-result-related-products"
                                        >
                                            <table className="table table-hover caption-top table-search-result table-related-products">
                                                <caption>
                                                    <button
                                                        ...
                                                        type="button"
                                                        className={
                                                            openSection.findIndex(
                                                                (o) => o === "relatedproducts"
                                                            ) < 0
                                                                ? "table-data-toggle collapse"
                                                                : "table-data-toggle expand"
                                                        }
                                                        aria-expanded={
                                                            openSection.findIndex(
                                                                (o) => o === "relatedproducts"
                                                        ...
                                                        aria-controls="tbody-data-resources"
                                                        role="button"
                                                        onClick={() => handleOpen("relatedproducts")}
                                                    >
                                                        {t("page.relatedproducts")}
                                                    </button>
                                                </caption>
                                                <tbody
                                                    id="tbody-related-products"
                                                    className={
                                                        openSection.findIndex(
                                                            (o) => o === "relatedproducts"
                                                        ) < 0
                                                    ...
                                                    aria-labelledby="related-products-id"
                                                >
                                                    <tr>
                                                    ...
                                                    {result.related.map((relatedp, ri) => {
                                                        ...
                                                        return (
                                                            <tr
                                                                ...
                                                                key={ri}
                                                                onClick={(e) =>
                                                                    handleRelatedClick(e, relatedp.id)
                                                                }
                                                            >
                                                                <td>
                                                                    <a
                                                                    ...
                                                                    >
                                                                        {relatedp[`description_${language}`]}
                                                                    </a>
                                                                </td>
                                    ...
                                    {/* Data Resources */}
                                    <section
                                        id="search-result-data-resources"
                                        className="sec-search-result sec-search-result-data-resources"
                                    >
                                        <table className="table table-hover caption-top table-search-result table-data-resources">
                                            <caption>
                                                <button
                                                    ...
                                                    aria-controls="tbody-data-resources"
                                                    role="button"
                                                    onClick={() => handleOpen("dataresources")}
                                                >
                                                    {t("page.dataresources")}
                                                </button>
                                            </caption>
                                            <tbody
                                                id="tbody-data-resources"
                                                className={
                                                    openSection.findIndex((o) => o === "dataresources") <
                                                        0
                                                        ? "collapse"
                                                        : "collapse show"
                                                }
                                                ...
                                                {options.map((option, oi) => {
                                                    ...
                                                    return (
                                                        <tr
                                                            className="table-row-link"
                                                            key={oi}
                                                            onClick={() => handleRowClick(option.url)}
                                                        >
                                                            <td>
                                                                <a
                                                                    className="table-cell-link"
                                                                    ...
                                                                    target="_blank"
                                                                    onClick={() =>
                                                                        resourceClick(option.name, option.type)
                                                                    }
                                                                >
                                                                    {option.name}
                                                                </a>
                                    ...
                                    {/* Contact Data */}
                                    <section
                                        id="search-result-contact-data"
                                        className="sec-search-result sec-search-result-contact-data"
                                    >
                                        <table className="table table-hover caption-top table-search-result table-contact-data">
                                            <caption>
                                                <button
                                                    ...
                                                    type="button"
                                                    className={
                                                        openSection.findIndex((o) => o === "contactdata") <
                                                            0
                                                            ? "table-data-toggle collapse"
                                                            : "table-data-toggle expand"
                                                    }
                                                    ...
                                                    aria-controls="tbody-contact-data"
                                                    onClick={() => handleOpen("contactdata")}
                                                >
                                                    {t("page.contactdata")}
                                                </button>
                                            </caption>
                                            <tbody
                                                id="tbody-contact-data"
                                                className={
                                                    openSection.findIndex((o) => o === "contactdata") < 0
                                                        ? "collapse"
                                                        : "collapse show"
                                                }
                                                aria-labelledby="conatct-data-id"
                                            >
                                                <tr>
                                    ...
                                    {/* Advanced Metadata */}
                                    <section
                                        id="search-result-adv-meta"
                                        className="sec-search-result sec-search-result-adv-meta"
                                    >
                                        <table className="table table-hover caption-top table-search-result table-adv-meta">
                                            <caption>
                                                <button
                                                    ...
                                                    type="button"
                                                    className={
                                                        openSection.findIndex((o) => o === "advdata") < 0
                                                            ? "table-data-toggle collapse"
                                                            : "table-data-toggle expand"
                                                    }
                                                    role="button"
                                                    ...
                                                    aria-controls="tbody-adv-meta"
                                                    onClick={() => handleOpen("advdata")}
                                                >
                                                    {t("page.advancedmetadata")}
                                                </button>
                                            </caption>
                                            <tbody
                                                id="tbody-adv-meta"
                                    ...
                                    <div className="top-sections">
                                        <section className="sec-search-result search-results-section search-results-map">
                                            <div className="ratio ratio-16x9">
                                                <Maplet id="metamap"
                                                    center={[(coordinates[0][1][0] + coordinates[0][0][0]) / 2, (coordinates[0][2][1] + coordinates[0][0][1]) / 2]}
                                                    zoom={zoom} coordinates={JSON.parse(result.coordinates)}
                                                />
                                            </div>
                                        </section>
                                        <section className="sec-search-result search-results-section search-results-misc-data">
                                            ...
                                            <div className="btn-group">
                                                <button
                                                    type="button"
                                                    className={`${mapButtonClass} mr-2`}
                                                    onClick={
                                                        activeMap
                                                            ? () => viewOnMap(event, result.id)
                                                            : () => setGreyMap(true)
                                                    }
                                                >
                                                    {t("page.viewonmap")}
                                                ...
                                                <button
                                                    ...
                                                    className={
                                                        inMapping
                                                            ? `${mapButtonClass} btn-added`
                                                            : mapButtonClass
                                                    }
                                                    onClick={
                                                        activeMap
                                                            ? () => changeMapping(result.id)
                                                            : () => setGreyMap(true)
                                                    }
                                                >
                                                    {inMapping
                                        ...
                                        <section className="sec-search-result search-results-section search-results-misc-data">
                                            ...
                                            <div className="btn-group">
                                                <a
                                                    href={`${EnvGlobals.APP_GEOCORE_URL}/${result.id}.geojson`}
                                                    className="btn btn-search mr-2"
                                                    rel="noreferrer"
                                                    target="_blank"
                                                    onClick={() => handleMetaDataBtn("geocore")}
                                                >
                                                    {t("page.downloadgeocore")}
                                                </a>
                                                {showWHDisclaimer ? (
                                                    <button
                                                        type="button"
                                                        ...
                                                        disabled
                                                    >
                                                        {t("page.viewhnaprecord")}
                                                    </button>
                                                ) : (
                                                    <a
                                                        href={`https://csw.open.canada.ca/geonetwork/srv/csw?service=CSW&version=2.0.2&request=GetRecordById&outputSchema=csw:IsoRecord&ElementSetName=full&id=${result.id}`}
                                                        className="btn btn-search"
                                                        rel="noreferrer"
                                                        target="_blank"
                                                        onClick={() => handleMetaDataBtn("hnap")}
                                                    >
                                                        {t("page.viewhnaprecord")}
                                                    </a>
                                                )}
                                    ...
                                    <div className="bottom-sections">
                                        <section className="sec-search-result search-results-section search-results-share-buttons">
                                            ...
                                            <div className="btn-group">
                                                ...
                                                <EmailShareButton url={encodeURI(window.location.href)}>
